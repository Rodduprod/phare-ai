...
published: true
qcm:
  - question: "Quelle est la différence fondamentale entre un chatbot et un agent IA ?"
    options:
      - "Un chatbot utilise GPT-4, un agent utilise Claude"
      - "Un agent peut agir sur le monde (utiliser des outils) en plus de générer du texte"
      - "Un chatbot est gratuit, un agent est payant"
      - "Un agent ne peut traiter que du code"
    answer: 1
  - question: "Dans le pattern ReAct, que signifie 'ACT' ?"
    options:
      - "L'agent génère du texte"
      - "L'agent choisit et exécute un outil (recherche web, API, base de données...)"
      - "L'agent termine la conversation"
      - "L'agent demande une confirmation à l'utilisateur"
    answer: 1
  - question: "Qu'est-ce que la 'mémoire à long terme' d'un agent IA ?"
    options:
      - "La RAM du serveur qui l'héberge"
      - "Des informations stockées dans une base vectorielle, persistantes entre les sessions"
      - "Le contexte de la conversation en cours"
      - "Les logs de toutes les conversations passées"
    answer: 1
  - question: "Pourquoi la supervision humaine reste-t-elle importante pour les agents IA ?"
    options:
      - "Parce qu'ils ne parlent pas français"
      - "Parce que leurs actions sont réelles et peuvent être irréversibles — et ils peuvent se tromper"
      - "Parce qu'ils sont trop lents sans supervision"
      - "Parce que la loi l'impose dans tous les pays"
    answer: 1
---
